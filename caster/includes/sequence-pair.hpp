...
struct Quadripartition{
	...
	void update(int x, int i){
		vector<thread> thrds;
		for (int p = 1; p < color.size(); p++) thrds.emplace_back(&Quadripartition::updatePart, this, p, x, i);
		updatePart(0, x, i);
	...
	array<double, 3> score(){
		...
		vector<thread> thrds;
		for (int p = 1; p < color.size(); p++) thrds.emplace_back(&Quadripartition::scorePart, this, p, ref(t[p]));
		scorePart(0, t[0]);
		for (thread &t: thrds) t.join();
		res[0] = 0;
		res[1] = 0;
	...
	CustomizedAnnotation annotate(){
		...
		vector<thread> thrds;
		for (int p = 1; p < color.size(); p++) thrds.emplace_back(&Quadripartition::annotatePart, this, p, ref(t[p]));
		annotatePart(0, t[0]);
		for (thread &t: thrds) t.join();
		for (int p = 0; p < color.size(); p++){
			res += t[p];
